...
import { textFilePath, createUploadUrl, doUpload, createDownloadUrl, doDownload, } from './fixtures';
...
test('basic download', async () => {
  ...
  const uploadResponse = await doUpload(createUploadUrlBody.uploadUrl, contentType, filePath);
